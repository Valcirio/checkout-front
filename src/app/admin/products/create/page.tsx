...
import { TRegisterProduct, ZRegisterProduct } from '@/validators/product'
...
export default function CreateProductPage() {
	...
	const {
		...
		criteriaMode: 'all',
		mode: 'onSubmit',
		defaultValues: { title: '', description: '', picture: '', price: 0, quantity: 1 },
	})

	async function handleProduct(data: TRegisterProduct) {
		try {
			const result = await instance.post('/product', data, {
				headers: {
			...
			<Card className="w-full max-w-xl">
				<CardHeader className="gap-1">
					...
					<CardDescription className="text-sm sm:text-lg">
						Em poucos passos, de maineira facilitada.
					</CardDescription>
				</CardHeader>
				<CardContent>
					<form
