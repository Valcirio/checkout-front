import ProductAdminSession from '@/components/productAdminSession'
...
export default async function UpdateProductPage({ params }: { params: Promise<{ uuid: string }> }) {
	try {
		...
		const result = await instance.get(`/product/admin/${uuid}`, {
			headers: { Authorization: `Bearer ${cookieData}` },
		})
		return <ProductAdminSession product={result.data.product} />
	} catch (err) {
